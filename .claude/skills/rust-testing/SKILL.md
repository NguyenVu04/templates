---
name: rust-testing
description: Guidance for testing Rust code - unit/integration/doc test structure, cargo-nextest, writing good unit and async tests, fakes vs mocks, property-based testing (proptest), snapshot testing (insta), CLI testing (assert_cmd), coverage, and test hygiene rules. Use this skill whenever the user is writing or reviewing Rust tests, setting up a test suite or test runner, adding property/snapshot/CLI tests, investigating flaky or slow tests, or measuring test coverage for a Rust project - even if they only mention "cargo", a Cargo.toml file, or .rs files without saying "Rust" explicitly.
---

# Rust Testing

Use this skill both when scaffolding a new project's test setup ("start a new Rust project" typically wants a testing scaffold offered alongside layout and CI) and when a build/test suite is reported as slow (nextest, test partitioning).

## Core principles that apply here

- **Verify before claiming.** After writing or editing Rust code, run `cargo check` (fast) and, if warranted, `cargo clippy --all-targets` and `cargo test` (or `cargo nextest run`). Never present code as working without at least a successful `cargo check`/`cargo test` when a toolchain is available. If no toolchain is available, say so explicitly.
- **Errors are API — test them as first-class.** Error paths deserve the same test coverage as happy paths, not an afterthought.

---
name: rust-release
description: Guidance for releasing and distributing Rust code - semver discipline, publishing to crates.io, docs.rs polish, binary distribution with cargo-dist, cross-compilation, Docker packaging for services, changelog discipline, and the release readiness gate. Use this skill whenever the user wants to publish a crate to crates.io, cut a release/version bump, cross-compile a Rust binary, package a Rust service as a Docker image, or prepare a changelog for a release - even if they only mention "cargo", a Cargo.toml file, or .rs files without saying "Rust" explicitly.
---

# Rust Release & Distribution

Use this skill for "ship it" requests: run the release readiness gate below, then walk through the crates.io publish checklist or binary/Docker distribution path as appropriate. If release automation (CI-driven releases) is also wanted, that's a separate CI/CD concern layered on top of this.

## Core principles that apply here

- **Verify before claiming.** Before any release, actually run the readiness gate commands (fmt, clippy, test, doc, deny, semver-checks) rather than assuming they'd pass. Never tell the user a release is ready without having run — or explicitly noting you couldn't run — these checks.
- **Pin what CI runs, float what users get.** MSRV (`rust-version`) is a public promise once published — raising it is a policy decision, not an incidental side effect of a dependency bump.
...
- Prefer showing complete, runnable files (full Dockerfile, full Cargo.toml metadata block) over fragments, unless editing an existing file.
- Respect existing project conventions (existing changelog format, versioning scheme) over this skill's defaults when working in an established codebase.

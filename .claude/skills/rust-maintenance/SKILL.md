...
description: Guidance for maintaining and evolving existing Rust codebases - triaging/auditing an unfamiliar or legacy project, dependency upgrade cadence, MSRV policy, edition migration, deprecation workflow for libraries, safely refactoring legacy Rust, and long-term health signals to monitor. Use this skill whenever the user wants to take over, audit, upgrade, or refactor an existing Rust project, asks about MSRV policy or edition migration (e.g. 2021 to 2024), or wants to deprecate/sunset a Rust crate - even if they only mention "cargo", a Cargo.toml file, or .rs files without saying "Rust" explicitly.
---

# Rust Maintenance & Evolution

Use this skill first for "upgrade / audit this old project" requests — it sequences the triage and the other work (dependency updates, code-quality fixes, CI setup) that follows from what the triage finds.

## Core principles that apply here

- **Verify before claiming.** Triage starts by actually running `cargo check`/`cargo test`/`cargo clippy` against the existing project, not assuming its state. Report what you actually ran and what it showed; never claim a legacy project builds or passes tests without having run the commands, or say so explicitly if you couldn't.
- **Let the toolchain do the enforcement.** Missing `rustfmt.toml`/`[lints]`/`deny.toml`/CI gates are exactly the kind of decay this skill exists to catch and fix.
- **Pin what CI runs, float what users get.** Check whether the toolchain is pinned and MSRV is declared as part of triage; these tend to rot first in unmaintained projects.
- **Edition 2024** is the target for new projects, but on an existing project respect its current edition and MSRV constraints — migrate deliberately, don't force it.
...
- Respect existing project conventions over this skill's defaults — maintenance work is inherently about working within an established codebase's choices, not replacing them wholesale.

...
description: Guidance for managing Rust dependencies - adding crates with correct version specifiers and features, choosing blessed-default crates (tokio, axum, serde, thiserror, etc.), evaluating unfamiliar crates, supply-chain policy via cargo-deny, keeping the dependency tree healthy, diagnosing slow builds, and designing feature flags. Use this skill whenever the user is adding or choosing a Rust crate/dependency, editing Cargo.toml dependency sections, asking about feature flags, investigating slow cargo builds, or setting up supply-chain checks like cargo-deny/cargo-audit - even if they only mention "cargo", a Cargo.toml file, or .rs files without saying "Rust" explicitly.
---

# Rust Dependency Management

Use this skill when a build or test suite is reported as slow (feature pruning, duplicate-dependency cleanup) or when the user is adding/choosing dependencies for a Rust project.

## Core principles that apply here

- **Let the toolchain do the enforcement.** Prefer configuration over convention: `deny.toml`, CI gates for supply-chain policy. A rule not enforced by a tool will decay.
- **Verify before claiming.** After changing dependencies, run `cargo check` (fast) and, if warranted, `cargo clippy --all-targets` and `cargo test`. Never present a dependency change as working without at least a successful `cargo check` when a toolchain is available. If no toolchain is available, say so explicitly.
- **Pin what CI runs, float what users get.** Commit `Cargo.lock` for binaries and workspaces (current guidance: commit it for libraries too). Specify major version only in Cargo.toml; let the lockfile handle exact reproducibility.
...
Most of the above are not genuinely contested choices (tokio vs async-std, for instance, isn't a real debate — use tokio). Where a choice is genuinely contested (axum vs actix-web), present the default with a one-line reason and mention the alternative rather than picking silently.
...
- `cargo outdated` or Renovate/Dependabot for upgrade PRs.

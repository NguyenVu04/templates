...
description: Guidance for Rust performance work - benchmarking with criterion/divan, profiling (flamegraph, samply, tokio-console, dhat), release profile tuning, common optimization patterns (allocation, hashing, parallelism, layout), and diagnosing bogus benchmark numbers. Use this skill whenever the user reports slow Rust code/services, asks to benchmark or profile Rust code, wants help tuning release profiles, or is investigating allocation/throughput/latency issues in a Rust program - even if they only mention "cargo", a Cargo.toml file, or .rs files without saying "Rust" explicitly.
---

# Rust Performance: Benchmarking, Profiling, Optimization

Use this skill for "my build/tests are slow" style performance complaints about runtime behavior (not compile time — that's a dependency-management concern), and generally whenever Rust code needs to be made faster.

## Core principles that apply here

- **Verify before claiming.** No optimization lands without a benchmark or profile showing (a) the code was hot and (b) the change helped — this is the performance-specific form of "verify before claiming." Never claim a change made something faster without measuring it; if you can't run a benchmark in this environment, say so explicitly.

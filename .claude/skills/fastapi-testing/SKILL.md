...
description: Guidance for testing FastAPI applications — pytest + pytest-asyncio + httpx.AsyncClient setup, fixture design (test DB, dependency overrides, authenticated client), what to test per endpoint, and coverage practice. Use this skill whenever the user wants to write or fix tests for a FastAPI endpoint, set up test fixtures/conftest.py, configure a test database, add an authenticated test client, or improve test coverage.
---

# FastAPI Testing

Guidance for testing FastAPI services thoroughly and fast, without hitting a real dev/prod database.

## Non-negotiable defaults

1. **Every new endpoint ships with a test.** Not optional. If an endpoint is added and no test, the task is incomplete.
2. **Async-first** — tests use `pytest-asyncio` and `httpx.AsyncClient`, matching the async application code.
3. Run `uv run pytest -x -q` after every implementation task; a task isn't done while tests are red.
...
Notes:
- `dependency_overrides` is THE mechanism for faking anything in tests — DB, current user, external clients. This is why anything you'll want to fake (current user, clock, external clients) should be defined as a FastAPI dependency rather than a module-level import.

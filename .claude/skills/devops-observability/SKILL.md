...
description: >-
  Production-grade observability engineering: Prometheus metrics and PromQL, structured logging,
  distributed tracing with OpenTelemetry, Grafana dashboards, alerting rules and Alertmanager, and
  SLOs/error budgets. Use this skill whenever the user mentions monitoring, alerting, metrics, logs,
  tracing, Prometheus, Grafana, OpenTelemetry, SLO, SLI, RED method, dashboards, or asks to "set up
  monitoring", "add alerts", "instrument this service", or debug why an incident wasn't caught by
  alerting — even if they don't say "observability" explicitly.
---

# Observability: Metrics, Logs, Traces, Alerts

A skill for producing production-grade observability configuration and making sound instrumentation decisions. The core theme: **boring, reproducible, observable, and reversible**. A clever alert rule is a liability; a predictable one is an asset.

## Before instrumenting anything

Establish (ask if not inferable):
- **Runtime & framework** — language/framework determines which OpenTelemetry auto-instrumentation applies (FastAPI/Express/Spring/Gin all have first-party support).
- **Target environment** — self-hosted on K8s vs a managed vendor (Grafana Cloud, Datadog, CloudWatch) changes the concrete stack, not the principles below.
- **Scale reality** — a single-service app doesn't need distributed tracing yet; add it once there are >2–3 services in a request path. Match complexity to actual need.
- **Existing conventions** — if the repo already emits metrics/logs in a particular shape, match it rather than imposing a new one.

## The mental model

Every service you help deploy should be able to answer three questions: **Is it up? Is it slow? Why?** Three signals, three jobs:
...
- Never log secrets, tokens, passwords, full card numbers. Redact at the logging-config layer, not by hoping — this is a non-negotiable security default, not a style preference.
...
- **Every page must be actionable and urgent.** If the response is "look at it Monday", it's a ticket, not a page. Alert fatigue is how real incidents get ignored — don't alert on everything.
...
- Health checks that always return 200 regardless of actual state are worse than no health check — they hide real failures from both probes and dashboards.
...
- Use Grafana provisioning (dashboards as JSON/Jsonnet in git), not hand-edited dashboards that die with the pod — dashboards and alert rules are code, and belong in version control with PR review like anything else.

## Output quality bar

- Deliver complete, runnable configs (alert rules, dashboard JSON, instrumentation snippets) — not fragments with `# ... rest of config` placeholders.
- Add brief comments explaining *why* a threshold or `for:` duration was chosen — not narrating YAML/PromQL syntax.
- When multiple valid approaches exist (self-hosted vs managed stack), pick one, deliver it, and mention the main alternative in one sentence with the tradeoff.
- After generating, mentally dry-run the artifact: does the alert expression's label set actually match what the app exports? Does the dashboard query the right job/service label?

## Anti-patterns to actively avoid

- Alerting on everything (alert fatigue) or health checks that always return 200.
- High-cardinality labels (user ID, request ID, raw path) blowing up Prometheus cardinality.
- Logging secrets or tokens in plaintext.
- Copy-pasting a "kitchen sink" dashboard or alert set with panels/rules the user didn't ask for and doesn't need.

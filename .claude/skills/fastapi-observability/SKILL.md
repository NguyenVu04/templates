...
description: Guidance for FastAPI logging, metrics, tracing, health checks, and production debugging — liveness/readiness endpoints, structured request logging, Prometheus RED metrics, OpenTelemetry tracing, and a playbook for diagnosing slow endpoints, timeouts, sporadic 500s, and memory creep. Use this skill whenever the user wants to add logging, metrics, tracing, health/readiness endpoints, or is debugging a production issue in a FastAPI service (slow requests, timeouts, 500s, memory growth).
---

# FastAPI Observability: Logs, Metrics, Health, Debugging

Guidance for making a FastAPI service observable and for diagnosing issues in production.
...
2. **Whole app slow / timeouts**: check event-loop blocking — a sync call snuck into an async path (a ruff `ASYNC` lint ruleset catches most of these ahead of time). Check pool saturation next.

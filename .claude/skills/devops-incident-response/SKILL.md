...
description: >-
  Production incident response and live-system debugging: triaging outages, diagnosing 5xx spikes,
  latency creep, connection failures, and restart storms, choosing mitigation levers (rollback,
  feature flag, scale, failover), running an incident, and writing blameless postmortems. Use this
  skill whenever the user mentions an outage, production incident, on-call, paging, rollback, SRE,
  postmortem, or asks to "debug production", "why is prod down", "the site is throwing 500s", or
  "we just had an incident, help me write it up" — even if they don't say "incident response"
  explicitly.
---

# Incident Response & Production Debugging

A skill for triaging live production incidents and writing blameless postmortems. The core theme: **boring, reproducible, observable, and reversible** — the fastest, safest lever wins during an incident; curiosity-driven debugging waits until the system is stable.
...
- Every deployment strategy needs a rollback story stated explicitly, and health checks (liveness/readiness) for anything serving traffic — an untested rollback path is a hypothesis, not a plan.
- Runbooks for every paging alert (symptoms → checks → levers). No runbook, no page.
- Alert on symptoms (user-facing error rate, latency), not causes (CPU%) — and only on actionable conditions. Alert fatigue from over-alerting is how real incidents get missed.
...
- When walking through a live incident, be concrete: name the exact command to run next, not a general category of action ("run `kubectl rollout undo deployment/api`", not "consider rolling back").
- When writing a postmortem, keep it factual and timestamped — avoid narrating intent ("we decided to...") when a log of actions and outcomes says more.
- Every recommended alert or runbook should be actionable: state what the on-call person actually does when it fires.

## Anti-patterns to actively avoid

- Debugging root cause before mitigating — extends outages for the sake of curiosity.
- Making multiple simultaneous changes during an incident (destroys the ability to attribute cause).
- Restarting a wedged process without capturing logs/heap first when that's cheap to do.
- Alerting on everything (alert fatigue) or health checks that always return 200 — both blind the team during the next incident.
- Postmortems that blame a person instead of the system that allowed one mistake to cascade.

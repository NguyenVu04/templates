---
name: rust-ci-cd
description: Guidance for Rust CI/CD on GitHub Actions - the baseline fmt/clippy/test/MSRV/deny/docs pipeline, optional jobs (feature powerset, miri, coverage, semver checks), caching strategy, release automation (release-plz, cargo-dist), and pipeline hygiene. Use this skill whenever the user is setting up or editing a GitHub Actions workflow for a Rust project, asking about CI caching or matrix builds, wiring up automated crate/binary releases, or configuring MSRV checks in CI - even if they only mention "cargo", a Cargo.toml file, or .rs files without saying "Rust" explicitly.
---

# Rust CI/CD (GitHub Actions)

Use this skill for "set up CI" requests standalone, or as a follow-up when scaffolding a new project (generate a minimal CI pipeline alongside the project layout), and for "ship it" requests that want release automation wired up.

## Core principles that apply here

- **Let the toolchain do the enforcement.** CI is where rules become real: `rustfmt.toml`, `clippy` lints in `[lints]`, `deny.toml` all need a CI gate or they decay. `RUSTFLAGS: -D warnings` at the env level makes check/test jobs strict; keep local builds lenient by not setting this outside CI.
- **Pin what CI runs, float what users get.** Pin the toolchain via `rust-toolchain.toml` (or explicit version in the MSRV job); commit `Cargo.lock`; declare and test `rust-version` (MSRV) in a dedicated CI job.
- **Verify before claiming.** The whole point of this pipeline is that `cargo check`/`clippy`/`test` actually ran and passed — never describe a pipeline as green without having seen it run, or say so explicitly if you haven't.

## Standard project baseline: required CI commands

These commands must pass locally and in CI for any project following this skill's conventions:
...
For containerized services, a `docker/build-push-action` job on tags pushing to GHCR.
...
- Prefer showing a complete, runnable `ci.yml` over fragments, unless editing an existing workflow file.
- Respect existing project conventions (existing workflow structure, job names) over this skill's defaults when working in an established codebase.

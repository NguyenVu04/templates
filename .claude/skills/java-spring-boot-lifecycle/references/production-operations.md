# Production Operations: Config, Kubernetes, JVM
...
- Anything secret comes from a Secret store (K8s Secret, Vault, cloud secret manager via `spring-cloud-*-config` or CSI driver). Verify at startup with validated `@ConfigurationProperties` — a pod that crash-loops on missing config is far better than one that limps.

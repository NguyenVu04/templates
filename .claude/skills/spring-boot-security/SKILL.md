...
description: >-
  Guidance for Spring Security in Java/Kotlin Spring Boot services:
  SecurityFilterChain configuration, choosing an auth model (OAuth2/JWT
  resource server, OIDC login, self-issued JWT, client credentials), method
  security, CORS, secrets handling, and security testing. Use this skill
  whenever the user is configuring authentication or authorization, adding
  JWT or OAuth2/OIDC support, writing @PreAuthorize rules, configuring CORS,
  handling password hashing or token issuance, hardening Actuator exposure,
  reviewing a service for BOLA/IDOR or secret-leakage risks, or writing
  security tests (401/403 paths) — including when the codebase clearly uses
  Spring Boot (pom.xml/build.gradle with spring-boot-starter-security,
...
This skill turns Claude into a reliable Spring Boot engineer for authentication, authorization, and hardening, so security-relevant code written in month 1 and month 18 looks like it came from the same team.
...
- **No secrets in the repo.** Not in `application.yml`, not in Dockerfiles, not in test fixtures. Use env vars / secret managers; document required variables in the README. `application.yml` holds placeholders only (`${DB_PASSWORD}`).
- **Fail fast on config**: add startup validation via `@ConfigurationProperties` so missing secrets kill boot instead of surfacing as a runtime auth failure later.
...
- Dependency CVE scanning in CI; Spring Security patch releases are applied within days, not quarters.
...
- **Write the negative tests: anonymous → 401, wrong role → 403, other user's resource → 404/403.** A security config without negative tests is unverified. Any security rule change ships with these tests in the same change — never merged untested.

## Working style expectations

- When reviewing code for security, check correctness → security → transaction boundaries → N+1/performance → naming/style, in that order, and report findings in that order.
- Explain *why* a security convention applies when it changes user-written code — the user should learn the reasoning (e.g., why deny-by-default, why row-level checks live in the service), not just accept edits.
